...
*/
import 'package:flutter/material.dart';
import 'package:image_picker/image_picker.dart'; // For QR code button
//import 'camera_page.dart';
import 'model_view_page.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> _scanQRCode() async {
    try {
      ...
      if (photo != null) {
        print('QR Image captured: ${photo.path}');
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('QR Image captured: ${photo.path}')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                const SizedBox(height: 16),
// AR Camera Button (emulator-friendly)
                ElevatedButton.icon(
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => const ModelViewPage(),
                      ),
                    );
                  },
                  icon: const Icon(Icons.threed_rotation),
                  label: const Text('View 3D Campus Model'),
                )
