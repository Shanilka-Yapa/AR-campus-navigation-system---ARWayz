...
class _GoogleMapPageState extends State<GoogleMapPage> {
  ...
  void _enterFacultyMode() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text("Entered Faculty Premises. Map View Expanded."),
        backgroundColor: Color(0xFF1A2D33),
      ),
    );

    // CHANGED: Instead of zooming to one point, we FIT the whole faculty bounds
    _mapController.fitCamera(
      CameraFit.bounds(
        bounds: _facultyBounds,
        padding: const EdgeInsets.only(
            top: 60, // Avoid back button
            left: 20,
            right: 20,
            bottom: 160 // Critical: Leave space for the bottom card list
        ),
      ),
    );
  }
  ...
  void _onPlaceSelected(int index) {
    ...
    // If a specific place is clicked, we zoom to THAT place
    // But user can still pinch out because we relaxed minZoom
    if (_isInsideFaculty) {
      _mapController.move(_facultyPlaces[index]['pos'], 18.0);
    } else {
      _fitBounds(_currentLocation!, _facultyPlaces[index]['pos']);
    }
  }
          ...
          FlutterMap(
            ...
            options: MapOptions(
              ...
              cameraConstraint: _isInsideFaculty
                  ? CameraConstraint.contain(bounds: _facultyBounds)
                  : const CameraConstraint.unconstrained(),
              // CHANGED: Lower minZoom so 'Fit Bounds' can zoom out enough to show everything
              minZoom: _isInsideFaculty ? 15.5 : 10.0,
              maxZoom: 22.0, // Allow user to zoom in very deep
            ),
            children: [
              TileLayer(
                urlTemplate: 'https://tile.openstreetmap.org/{z}/{x}/{y}.png',
